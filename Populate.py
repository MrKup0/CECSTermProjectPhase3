...
def thank_you_python():
    # Populate Employees
    employees = db.employees
    #validator = Setup.employees_validator
    # db.command('collMod', 'employees', **validator)
    ...
    #validator = Setup.buildings_validator
    # db.command('collMod', 'buildings', **validator)
    ...
    buildings = db.buildings
    rooms = db.rooms
    # validator = Setup.rooms_validator
    # db.command('collMod', 'rooms', **validator)
...
issued_keys.insert_many([
    {'request_id': DBRef('requests', requests[0]['_id']),
     'key_id': DBRef('keys', keys[]),
     'date_due_back': datetime.now() + datetime.timedelta(year=1),
     'date_returned': None,
     'date_lost': None}
])